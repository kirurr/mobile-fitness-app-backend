...
import { userCompletedProgramService } from "./service";
import { 
...
userCompletedProgramRouter.get("/", jwtMiddleware, async (c) => {
  try {
...
userCompletedProgramRouter.get("/:id", jwtMiddleware, async (c) => {
  try {
...
userCompletedProgramRouter.post("/", jwtMiddleware, zValidator("json", createUserCompletedProgramSchema), async (c) => {
  try {
    const userId = c.var.user.id;
...
userCompletedProgramRouter.put("/:id", jwtMiddleware, zValidator("json", updateUserCompletedProgramSchema), async (c) => {
  try {
    const id = Number(c.req.param("id"));
    if (isNaN(id)) {
      return c.json({ error: "Invalid program ID" }, 400);
...
userCompletedProgramRouter.delete("/:id", jwtMiddleware, async (c) => {
  try {
    const id = Number(c.req.param("id"));
    if (isNaN(id)) {
      return c.json({ error: "Invalid program ID" }, 400);
