...
plannedExerciseProgramRouter.get("/", jwtMiddleware, async (c) => {
  try {
...
plannedExerciseProgramRouter.get("/:id", jwtMiddleware, async (c) => {
  try {
...
plannedExerciseProgramRouter.post("/", jwtMiddleware, zValidator("json", createPlannedExerciseProgramSchema), async (c) => {
  try {
    const userId = c.var.user.id;
...
plannedExerciseProgramRouter.put("/:id", jwtMiddleware, zValidator("json", updatePlannedExerciseProgramSchema), async (c) => {
  try {
    const id = Number(c.req.param("id"));
    if (isNaN(id)) {
      return c.json({ error: "Invalid program ID" }, 400);
...
plannedExerciseProgramRouter.delete("/:id", jwtMiddleware, async (c) => {
  try {
    const id = Number(c.req.param("id"));
    if (isNaN(id)) {
      return c.json({ error: "Invalid program ID" }, 400);
...
plannedExerciseProgramRouter.get("/:programId/dates", jwtMiddleware, async (c) => {
  try {
...
plannedExerciseProgramRouter.post("/:programId/dates", jwtMiddleware, zValidator("json", createPlannedExerciseProgramDateSchema), async (c) => {
  try {
    const programId = Number(c.req.param("programId"));
    if (isNaN(programId)) {
      return c.json({ error: "Invalid program ID" }, 400);
...
plannedExerciseProgramRouter.get("/:programId/dates/:dateId", jwtMiddleware, async (c) => {
  try {
    const programId = Number(c.req.param("programId"));
...
plannedExerciseProgramRouter.put("/:programId/dates/:dateId", jwtMiddleware, zValidator("json", updatePlannedExerciseProgramDateSchema), async (c) => {
  try {
    const programId = Number(c.req.param("programId"));
    const dateId = Number(c.req.param("dateId"));
    if (isNaN(programId) || isNaN(dateId)) {
...
plannedExerciseProgramRouter.delete("/:programId/dates/:dateId", jwtMiddleware, async (c) => {
  try {
    const programId = Number(c.req.param("programId"));
    const dateId = Number(c.req.param("dateId"));
    if (isNaN(programId) || isNaN(dateId)) {
